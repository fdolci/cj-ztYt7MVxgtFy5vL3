...
/*
                ...
                $y = parse_url($archivo);
                $ruta = str_replace( '/'.$name  , '' , $y['path'] ).'/';
                $ruta = substr($ruta,1,strlen($ruta));

                $ruta_absoluta = $_SERVER['DOCUMENT_ROOT'].$ruta;
                $thumbs = hace_thumb($name,$ruta_absoluta);
 */               
                ...
                if ($foto_id == 0) {
                    $cond = "insert into departamentos_fotos (departamento_id, nombre1,nombre2,nombre3,nombre4,nombre5, archivo, thumbs, alto, ancho) values 
    				        ('$dpto_id', '$nombre1','$nombre2','$nombre3','$nombre4','$nombre5', '$archivo', '$tn','$alto','$ancho')";
    				$ok   = $db->Execute($cond);
                } else {
                    $cond = "update departamentos_fotos set nombre1='$nombre1', nombre2='$nombre2',nombre3='$nombre3',nombre4='$nombre4',nombre5='$nombre5', 
			...
			case 'foto_eliminar':
				$sql = "delete from departamentos_fotos where id='$foto_id'";
				$rs  = $db->Execute($sql);
			...
			case 'foto_estado':
				if ($foto_id>0){
					$sql = "select * from departamentos_fotos where id='$foto_id'";
					$rs  = $db->SelectLimit($sql,1);
					$x   = $rs->FetchRow();
					$activo = iif($x['activo']==0,1,0);
					$sql = "update departamentos_fotos set activo='$activo' where id='$foto_id'";
					$rs 	= $db->Execute($sql);
				}
				break;
			...
			case 'foto_orden':
				if ($foto_id>0){
					$cond 	= "select * from departamentos_fotos where id=".$foto_id;
					...
					$cond 	= "update departamentos_fotos set orden=".$nuevo_orden." where id=".$foto_id;
					$ok		= $db->Execute($cond);
					...
					$orden = 1;
					foreach ($x as $r){
						$cond 	= "update departamentos_fotos set orden=".$orden." where id=".$r[id];
						$ok		= $db->Execute($cond);
						$orden++;
					}
				}
...
*/    
?>
			<?php include_once('galeria_galeria.php');?>
