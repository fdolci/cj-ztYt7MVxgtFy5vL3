<?php
	...
	if($producto_id<1){ die('Hacking attempt!!'); }
    ...
    foreach($Imagenes as $clave=>$valor){
        if ($valor['cual']=='logo_evento'){
            ...
            unset($Imagenes[$clave]);
        } elseif ($valor['cual']=='banner'){
            $Producto['banner'] = VER_FOTOS.'/'.$valor['imagen'];    
            unset($Imagenes[$clave]);
        }
    }
    ...
    //----------------------------------------------------------------
    //                                   Obtiene Familia/Subfamilia
    //----------------------------------------------------------------
    $sql = "select pf.familia_id, pf.subfamilia_id, fam.nombre1 as nombre_fam , sub.nombre1 as nombre_sub
			from productos_familias as pf 
			left join familias as fam on fam.id=pf.familia_id
			left join familias as sub on sub.id=pf.subfamilia_id
			where pf.producto_id='$producto_id'";
    $rs  = $db->SelectLimit($sql,1);
    $x = $rs->FetchRow();
	$xurl['familia']['nombre'] = $x['nombre_fam'];
	$xurl['subfamilia']['nombre'] = $x['nombre_sub'];
    ...
    //                                   Hay Alojamientos ?
    //----------------------------------------------------------------
    $sql = "select * from productos_alojamientos where producto_id='$producto_id' order by orden ASC";
    $rs  = $db->Execute($sql);
...
//pr($xurl);
    ...
    $oculta_todo = true;
    include_once(ROOT.'/api/full/header.html.php');
    include_once(ROOT.'/html/muestra_comercio.html.php');
    include_once(ROOT.'/api/full/footer.html.php');
//pr($xurl);
    die();
?>