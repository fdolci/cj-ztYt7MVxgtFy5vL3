...
*/
	...
	switch ($accion) {
	    case 'guardar':
		    if($_POST){
				...
				if ($_FILES['archivo']['size'] != 0){
					...
					if (! move_uploaded_file ($_FILES['archivo']['tmp_name'], SUBIR_ALOJAMIENTOS."/".$NombreArchivo)) {
				...
				unset($data['accion']);
				$db->debug = false;
		        $ok = $db->Replace('productos_alojamientos', $data,'id', $autoquote = true); 
	    ...
	    case 'estado':
	    	$rs = $db->SelectLimit("select activo from productos_alojamientos where id='$id'",1);
	    	$x  = $rs->FetchRow();
	    	$estado = iif($x['activo']==1,0,1);

	    	$ok = $db->Execute("update productos_alojamientos set activo='$estado' where id='$id'");
			$id = 0;	        
	        break;

	    case 'editar':
	    	$rs = $db->SelectLimit("select * from productos_alojamientos where id='$id'",1);
	    	$data  = $rs->FetchRow();
	        break;

	    case 'eliminar':
	    	$rs = $db->SelectLimit("select archivo from productos_alojamientos where id='$id'",1);
	    	$x  = $rs->FetchRow();
	    	$archivo = $x['archivo'];

	    	$ok = $db->Execute("delete from productos_alojamientos where id='$id'");

	    	if (file_exists ( SUBIR_ALOJAMIENTOS.'/'.$archivo) ) {
	    		unlink(SUBIR_ALOJAMIENTOS.'/'.$archivo);
	    	}
	    ...
	    case 'ordenar':
	    	...
	    	foreach($_GET['listItem'] as $clave=>$valor){
				$ok = $db->Execute("update productos_alojamientos set orden='$clave' where id='$valor'");	    		
	    	}
	    	die();
			$id = 0;	        
	        break;
	...
	$sql = "select * from productos_alojamientos where producto_id='$producto_id' and user_id='$user_id' order by orden ASC";
	$rs  = $db->Execute($sql);
    $ax = $rs->GetRows();
...
<form name='archivos' action='<?php echo URL;?>/account/aviso_alojamientos.html.php?producto_id=<?php echo $producto_id;?>' method='post' enctype="multipart/form-data" >
<fieldset>
	<label for='titulo' style='display:inline-block;line-height:30px;height:30px;'>Nombre:</label>
	<input type='text' name='titulo' id='titulo' value='<?php echo $data['titulo'];?>' style='padding:3px; line-height:30px; height:30px; display:inline-block;width:500px;'/>
</fieldset>
<fieldset>
	<label for='titulo' style='display:inline-block;line-height:30px;height:30px;'>URL:</label>
	<input type='text' name='href' id='href' value='<?php echo $data['href'];?>' style='padding:3px; line-height:30px; height:30px; display:inline-block;width:500px;'/>
</fieldset>

<fieldset>
	<label for='descripcion' style='line-height:30px;height:30px;display:inline-block;'>Descripción:</label>
	<textarea name='descripcion' id='descripcion' style='width:480px; height:100px;' ><?php echo $data['descripcion'];?></textarea>    
</fieldset>

<fieldset>
	<input type='file' name='archivo' id='archivo' value='' style='padding:3px; height:30px; display:inline-block;'/>
	<br><small>Tamaño máximo permitido: 3Mb. (350px * 350px)</small>
  ...
  $(document).ready(function() {
    $("#test-list").sortable({
      ...
      update : function () {
		...
		$("#info").load("<?php echo URL;?>/account/aviso_alojamientos.html.php?producto_id=<?php echo $producto_id;?>&"+order);
      }
    });
});
</script>
...
<?php if (!empty($ax)) { ?>
	...
	<table width='100%' style="border-width:1px; border-style:solid; border-color:#001860;" cellpadding='8' cellspacing="1">
		<tr bgcolor='#001860'>
			...
			<td style="color:#FFFFFF; font-weight:bold; font-size:12px; text-align:right;">Links</td>
		</tr>
	</table>	
...
*/?>							
					...
					<td width='80' align='center' style="color:#000000; font-weight:bold; font-size:12px;">
						<a href='aviso_alojamientos.html.php?accion=eliminar&producto_id=<?php echo $producto_id;?>&id=<?php echo $x['id'];?>'
							title='Eliminar este Archivo'
							onclick="return confirm('Est&aacute; seguro de eliminar este Archivo?');">
							<img src='<?php echo ADMIN;?>img/del.gif' border='0' /></a>&nbsp;&nbsp;&nbsp;&nbsp;

						<a href='aviso_alojamientos.html.php?accion=estado&producto_id=<?php echo $producto_id;?>&id=<?php echo $x['id'];?>' title='Activar / Desactivar' >
							<?php if ($x['activo']==1){ echo "<img src='".ADMIN."img/activo.gif' border='0' />";
							} else { echo "<img src='".ADMIN."img/inactivo.gif' border='0' />"; }?>
						</a>
						&nbsp;&nbsp;&nbsp;&nbsp;
						<a href='aviso_alojamientos.html.php?accion=editar&producto_id=<?php echo $producto_id;?>&id=<?php echo $x['id'];?>' title='Editar este Archivo'>
							<img src='<?php echo ADMIN;?>img/edit.gif' border='0' /></a>
					</td>

					<td style="text-align:center;" valign='middle'>
						<img src='<?php echo VER_ALOJAMIENTOS.'/'.$x['archivo'];?>' style='max-width:70px;width:70px; height:70px;padding-right:10px;' title="<?php $x['descripcion'];?>" />
					</td>
					<td style="color:#000000; font-weight:bold; font-size:12px;" valign='middle'>
						<?php echo $x['titulo'];?><small style='font-weight:normal;'><br><?php echo $x['descripcion'];?></small>
					</td>
					<td style="color:#000000; font-weight:bold; font-size:12px; text-align:right;margin-right:20px; width:30px;" valign='middle'>
