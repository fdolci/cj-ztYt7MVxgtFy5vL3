	...
	switch ($accion) {
	    case 'guardar':
		    if($_POST){
				...
				unset($data['cargo']);
//				$db->debug = true;
		        $ok = $db->Replace('productos_disertantes', $data,'id', $autoquote = true); 
//				die();
		        unset($data);
	    ...
	    case 'estado':
	    	$rs = $db->SelectLimit("select activo from productos_disertantes where id='$id'",1);
	    	$x  = $rs->FetchRow();
	    	$estado = iif($x['activo']==1,0,1);

	    	$ok = $db->Execute("update productos_disertantes set activo='$estado' where id='$id'");
			$id = 0;	        
	        break;

	    case 'editar':
	    	$rs = $db->SelectLimit("select * from productos_disertantes where id='$id'",1);
	    	$data  = $rs->FetchRow();
	    ...
	    case 'eliminar':
	    	$rs = $db->SelectLimit("select archivo from productos_disertantes where id='$id'",1);
	    	$x  = $rs->FetchRow();
	    	$archivo = $x['archivo'];

	    	$ok = $db->Execute("delete from productos_disertantes where id='$id'");
	    ...
	    case 'ordenar':
	    	...
	    	foreach($_GET['listItem'] as $clave=>$valor){
				$ok = $db->Execute("update productos_disertantes set orden='$clave' where id='$valor'");	    		
	    	}
	    	die();
			$id = 0;	        
	        break;
	...
	$sql = "select * from productos_disertantes where producto_id='$producto_id' and user_id='$user_id' order by orden ASC";
	$rs  = $db->Execute($sql);
    $ax = $rs->GetRows();
