<?php
    ...
    $mDescription    = date("d/m",$Producto['desde']).' al '. date("d/m/Y",$Producto['hasta']).' en ';
	$mDescription    .= $Producto['lugar'].' - '.$Producto['ciudad'].' - '.$Producto['provincia'].' - ';
	$mDescription    .= $Producto['subtitulo'];
	$mCreated        = date("Y-m-d H:i:s");	
    ...
    //                                   Hay Gastronomia ?
    //----------------------------------------------------------------
    $sql = "select * from productos_gastronomia where producto_id='$producto_id' order by orden ASC";
    $rs  = $db->Execute($sql);
    $Gastronomia = $rs->GetRows();
